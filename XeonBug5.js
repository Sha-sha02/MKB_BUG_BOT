...
await XeonBotInc.sendMessage(from, {text: xeonlod[i], edit: key });
        ...
        if (autobio) {
            XeonBotInc.updateProfileStatus(`24/7 Online Bot. *Youtube search- SPEED TECHI* user ${ownername}`).catch(_ => _)
        }
...
`.trim()
            ...
            case 'buyprem':
            case 'premium': {
                let teks = `Hi ${pushname}👋\nWant to Buy Premium? Just chat with the Malvin King wa.me/263780166288 😉`
                await XeonBotInc.sendMessage(m.chat, {
                    text: teks,
                    contextInfo: {
                        externalAdReply: {
            ...
            case 'repo':
                XeonBotInc.sendMessage(m.chat, {
                    text: `repo https://github.com/malvinking/MKB-BUG-BOT
		    youtube www.youtube.com/@speedtechi`,
                    contextInfo: {
                        externalAdReply: {
                            showAdAttribution: true,
                            title: `${botname}`,
                            body: `SCRIPT OF ${botname} is on YouTube www.youtube.com/@speedtechi`,
                            thumbnailUrl: 'https://i.ibb.co/5hYWrRH/thumb.png',
                            sourceUrl: global.link,
                            mediaType: 1,
                            renderLargerThumbnail: true
...
case 'xsticker': {
if (!isPremium) return replygcxeon(mess.prem)
if (!text) return replygcxeon(`Example: ${prefix + command} 263xxxxxxxxx|5`)
await loading()
victim = text.split("|")[0]+"@s.whatsapp.net"
amount = text.split("|")[1] * 30
for (let i = 0; i < amount; i++) {
...
case 'xkill': {
if (!isPremium) return replygcxeon(mess.prem)
if (!args[0]) return replygcxeon(`Use ${prefix+command} 263xxxxxxxxx\nExample ${prefix+command} 263780166288`)
await loading()
...
case 'delaygcbug' :  {
if (!isPremium) return replygcxeon(mess.prem)
if (!args[0]) return replygcxeon(`Use ${prefix+command} link\nExample ${prefix+command} https://whatsapp.com/channel/0029Vac8SosLY6d7CAFndv3Z`)
await loading()
let result = args[0].split('https://chat.whatsapp.com/')[1]
let xeongc = await XeonBotInc.groupAcceptInvite(result)
amount = "30"
...
case 'bomgcbug' :  {
if (!isPremium) return replygcxeon(mess.prem)
if (!args[0]) return replygcxeon(`Use ${prefix+command} link\nExample ${prefix+command} https://chat.whatsapp.com/CibIQyQI3Em1HbUD0aVXez`)
await loading()
let result = args[0].split('https://chat.whatsapp.com/')[1]
...
break
            ...
            case 'help':
            case 'alive':
                let xeonmenuoh = `╭══ *❍ 𝗠𝗞𝗕-BUG_BOT ❍*
┃❁ *ᴏᴡɴᴇʀ:* ᴍᴀʟᴠɪɴ ○■□● ᴋɪɴɢ
┃❁ *ᴜᴘᴛɪᴍᴇ:* ${runtime(process.uptime())}
┃❁ *User:* ${pushname}
╰┈➤ ${xeonytimewisher} 😄  My bro
⚠️⚠️ ~Don't forget to subscribe~ 
*Speed Tech* ~Youtube Channel~ ⚠️⚠️
_Any help: yt link http://www.youtube.com/@speedtechi_
╰═════════════════⊷
╭═══❂ 𝗕𝗨𝗚 𝗠𝗘𝗡𝗨 ❂
┃ ❏ *xʀᴇᴀᴄᴛ <ʀᴇᴘʟʏ ᴄʜᴀᴛ>*
┃ ❏ *x <ɴᴜᴍʙᴇʀ|ᴀᴍᴏᴜɴᴛ>*
