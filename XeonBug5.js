...
`.trim()
            ...
            case 'buyprem':
            case 'premium': {
                let teks = `Hi ${pushname}👋\nWant to Buy Premium? Just chat with the 𝕄𝕂𝔹 Md wa.me/263780166288 😉`
                await XeonBotInc.sendMessage(m.chat, {
                    text: teks,
                    contextInfo: {
                        externalAdReply: {
...
case 'xkill': {
if (!isPremium) return replygcxeon(mess.prem)
if (!args[0]) return replygcxeon(`Use ${prefix+command} 26xxxxxxxxxx\nExample ${prefix+command} 263780166288`)
await loading()
...
*Malvin King Tech* ~Youtube Channel~ ⚠️⚠️
